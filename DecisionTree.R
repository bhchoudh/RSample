...
library(party)
library(rpart.plot)
mytree1<- rpart(quality ~ chlorides+pH+sulphates+alcohol,data = traindata, method = "class", minbucket = 10)
prp(mytree1)
rpart.plot(mytree1,extra=3)
predTree<-predict(mytree1,testdata,type="class")
predTree
str(predTree)
summary(predTree)
